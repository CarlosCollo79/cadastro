...
def check_page(file_path: Path) -> dict:
    ...
    is_layout = 'Head>' in content or '<head>' in content.lower() or '<head ' in content.lower() or 'export const metadata' in content or 'export default function RootLayout' in content
    
    # 1. Title tag
    has_title = '<title' in content.lower() or 'title=' in content or 'Head>' in content or ('metadata' in content and 'title' in content.lower())
    if not has_title and is_layout:
        issues.append("Missing <title> tag")
    
    # 2. Meta description
    has_description = 'name="description"' in content.lower() or 'name=\'description\'' in content.lower() or ('metadata' in content and 'description' in content.lower())
    if not has_description and is_layout:
        issues.append("Missing meta description")
    
    # 3. Open Graph tags
    has_og = 'og:' in content or 'property="og:' in content.lower() or ('metadata' in content and 'openGraph' in content)
